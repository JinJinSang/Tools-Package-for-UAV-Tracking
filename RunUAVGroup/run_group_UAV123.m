...
function run_group_UAV123(save_dir)                                        % 没有输入，则默认保存在当前文件下下的\all_trk_results\
%%  读取视频序列和groundtruth
where_is_your_groundtruth_folder = 'D:\BaiduNetdiskDownload\UAV123\anno\UAV123';        % 包含所有数据集groundtruth文件的路径
...
%UAV123数据集共123个视频序列，总共112578帧图像，平均序列长度为915.27帧
...
if nargin < 1
    save_dir = [main_folder '\group_results\UAV123\'];              % 保存跑完的结果到指定文件夹
end
