...
function run_group_UAV123_20L(save_dir)                                        % 没有输入，则默认保存在当前文件下下的\all_trk_results\
%%  读取视频序列和groundtruth
where_is_your_groundtruth_folder = 'D:\BaiduNetdiskDownload\UAV123\anno\UAV20L';        % 包含所有数据集groundtruth文件的路径
where_is_your_UAV123_database_folder = 'D:\BaiduNetdiskDownload\UAV123\data_seq\UAV123';       % 包含所有数据集图片序列的路径
%UAV123数据集的视频序列和groundtruth是分开的
%UAV123_20L数据集共20个视频序列，总共58670帧，平均序列长度2933.5帧
...
if nargin < 1
    save_dir = [main_folder '\group_results\UAV123_20L\'];              % 保存跑完的结果到指定文件夹
end
