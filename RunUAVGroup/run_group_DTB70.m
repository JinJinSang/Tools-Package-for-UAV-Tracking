...
function run_group_DTB70(save_dir)                                        % 没有输入，则默认保存在当前文件下下的\all_trk_results\
%%  读取视频序列和groundtruth
where_is_your_DTB70_database_folder = 'D:\BaiduNetdiskDownload\DTB70'; % 包含所有数据集图片序列和groundtruth的路径
%DTB70数据集的视频序列和groundtruth是在一起的
%DTB70共有70个数据集，总共15777帧图像，平均序列长度225.39帧
addpath('.\UAV_Basic_Framework\');

%% Read all video names using grouthtruth.txt
dir_output = dir(fullfile(where_is_your_DTB70_database_folder));             % 获取该文件夹下的所有的txt文件
contents = {dir_output.name}';
all_video_name = cell(numel(contents)-2,1);
for k = 3:numel(contents)
    name = contents{k}(1:end);                                       % 去掉后缀 .txt
    all_video_name{k-2,1} = name;                                    % 保存所有数据集名称
end
...
if nargin < 1
    save_dir = [main_folder '\group_results\DTB70\'];              % 保存跑完的结果到指定文件夹
end
run_trackers_info = struct('ECO_HC',@run_ECO_HC);                               % 获取运行tracker的函数信息，函数最好是run_xxx(seq, res_path, bSaveImage))这种
...
for dataset_count =1:dataset_num
    ...
    seq = load_video_info_DTB70([database_folder '\' video_name]);% 加载序列信息
