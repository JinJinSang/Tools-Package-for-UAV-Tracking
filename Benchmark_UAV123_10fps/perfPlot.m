...
dataPath ='D:\BaiduNetdiskDownload\UAV123_10fps\data_seq';
annoPath = ['.\anno\', 'UAV123_10fps' , '\']; 
seqs_name='UAV123_10fps';
% 可选项
...
figWidth =800;% 图片宽度    1500  800
figHeight =600; % 图片高度      385  600
...
for i=1:length(metricTypeSet)
    ...
    for j=1:length(evalTypeSet)
        ...
        switch metricType
            case 'overlap'
                titleName = ['Success plots of ' evalType ' - ' 'Success plots on UAV123'];
            case 'error'
                titleName = ['Precision plots of ' evalType ' - ' 'Precision plots on UAV123'];
        end
...
%             saveas(gcf, [figsNameTosave, '.png']); % 保存成 png，或 tiff
        ...
        for attIdx=1:attNum
            ...
            switch metricType
                case 'overlap'
                    titleName = ['Success plots of ' seqs_name evalType ' - ' attName{attIdx} ' (' num2str(length(idxSeqSet)) ')'];
                case 'error'
                    titleName = ['Precision plots of ' seqs_name evalType ' - ' attName{attIdx} ' (' num2str(length(idxSeqSet)) ')'];
            end
