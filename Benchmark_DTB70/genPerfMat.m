function genPerfMat(seqs, trackers, evalType, nameTrkAll, perfMatPath, annoPath)
...
switch evalType
    case 'SRE'
        rpAll = fullfile(cur_dir, 'results', 'results_SRE');
    case {'TRE'}
        rpAll = fullfile(cur_dir, 'results', 'results_TRE');
    case {'OPE'}
        rpAll = fullfile(cur_dir, 'results', 'results_OPE');
end
...
for idxSeq=1:length(seqs)
    ...
    for idxTrk=1:numTrk
        ...
        load(fullfile(rpAll, [t.name '\' s.name '_' t.name '.mat']));
        disp([s.name ' ' t.name]);
...
dataName1=fullfile(perfMatPath, ['aveSuccessRatePlot_' num2str(numTrk) 'alg_overlap_' evalType '.mat']);
...
dataName2=fullfile(perfMatPath, ['aveSuccessRatePlot_' num2str(numTrk) 'alg_error_' evalType '.mat']);
