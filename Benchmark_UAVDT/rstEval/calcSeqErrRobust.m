...
switch results.type
    ...
    case 'ivtAff'
        for i = 1:seq_length
            [rect, ~] = calcRectCenter(results.tmplsize, results.res(i,:));
            rectMat(i,:) = rect;
        end
    case 'L1Aff'
        for i = 1:seq_length
            [rect, ~] = calcCenter_L1(results.res(i,:), results.tmplsize);
            rectMat(i,:) = rect;
        end
    case 'LK_Aff'
        for i = 1:seq_length
            [corner, ~] = getLKcorner(results.res(2*i-1:2*i,:), results.tmplsize);
    ...
    case 'SIMILARITY'
        for i = 1:seq_length
            ...
            [corner , ~] = getLKcorner(warp_p, results.tmplsize);
            rectMat(i,:) = corner2rect(corner);
        end
end
