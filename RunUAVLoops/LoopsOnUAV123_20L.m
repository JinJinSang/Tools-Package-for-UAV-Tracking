 ...
 function LoopsOnUAV123_20L(save_dir)                                        % 没有输入，则默认保存在当前文件下下的\all_trk_results\
%%  读取视频序列和groundtruth
...
cd(tracker_name);         
if nargin < 1
    save_dir = [main_folder '\loop_results\UAV123_20L\'];              % 保存跑完的结果到指定文件夹
end
%%
% 需要调的参数系列值以及参数名(手动输入)
